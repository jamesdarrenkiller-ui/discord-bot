...
function registerEvents(client) {
  ...
  client.on('messageCreate', async message => {
    ...
    if(commandName==='help'||commandName==='h') return message.reply(['🤖 **All-in-One Bot — Prefix Help**','',`Prefix: **${prefix}**`,'','🛡️ **Moderation** — `ban`, `kick`, `timeout`, `untimeout`, `warn`, `warnings`, `unwarn`, `warnings-clear`, `clear`, `lock`, `unlock`, `unban`, `softban`, `slowmode`, `nick`, `deafen`, `undeafen`, `voicekick`, `modlogs`','🛡️ **Security** — `automod`, `antilink`, `anticaps`, `antiraid`, `antinuke`','💰 **Economy** — `balance`, `daily`, `weekly`, `work`, `beg`, `pay`, `leaderboard`','🎵 **Music** — `play`, `pause`, `resume`, `skip`, `stop`, `queue`, `volume`, `loop`, `shuffle`, `mode`, `modes`','🎫 **Tickets** — `ticket`','🔧 **Utility** — `ping`, `serverinfo`, `userinfo`, `avatar`, `botinfo`, `uptime`','🧠 **AI** — `ai`','🏷️ **Tags** — `tag <name>`','⏰ **Reminders** — `/remind`','📊 **Polls** — `/poll create`',''].join('\n'));
